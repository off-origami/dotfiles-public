...
-- I use it to start my dev inv (just splits panes on 4)
wezterm.on("dev", function(window, pane)
  ...
  -- Perform the initial horizontal split
    window:perform_action(wezterm.action.SplitPane({
    direction = "Right",
    size = {Percent = 66}
  }), pane)
...
local keys = {
   ...
   -- panes --
   -- panes: split on 4 screens
   {
      key = [[d]],
      mods = mod.SUPER_REV,
      action = wezterm.action.EmitEvent("dev"),
   },
   -- panes: split panes
   {
      key = [[/]],
